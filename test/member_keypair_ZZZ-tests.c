...
void member_secret_is_valid()
{
    ...
    ecdaa_member_key_pair_ZZZ_generate(&pk1, &sk1, nonce, sizeof(nonce), test_randomness);

    TEST_ASSERT(!pk1.Q.inf);

    struct ecdaa_member_secret_key_ZZZ sk2;
    struct ecdaa_member_public_key_ZZZ pk2;
    ecdaa_member_key_pair_ZZZ_generate(&pk2, &sk2, nonce, sizeof(nonce), test_randomness);
...
void member_public_is_valid()
{
    ...
    uint8_t nonce[32] = {0};
    ecdaa_member_key_pair_ZZZ_generate(&pk, &sk, nonce, sizeof(nonce), test_randomness);
...
static void zero_nonce_ok()
{
    ...
    struct ecdaa_member_secret_key_ZZZ sk;
    struct ecdaa_member_public_key_ZZZ pk;
    ecdaa_member_key_pair_ZZZ_generate(&pk, &sk, NULL, 0, test_randomness);
