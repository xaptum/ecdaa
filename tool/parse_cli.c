...
void parse_process_response_cli(int argc, char **argv, struct cli_params *params)
{
    ...
    const char *usage_str = "Process the response to our join request.\n\n"
        ...
        "\t\t-r --credsig           Secret key output location [default = cred_sig.bin]\n"
        ;
...
void parse_member_cli(int argc, char** argv, struct cli_params *params)
{
    const char *usage_str =
        "Options:\n"
        "\trequestjoin                   Generate public/secret key pair from nonce.\n"
        "\tprocessresponse               Process response by issuer.\n"
        "\tsign                          Respond to join request by member.\n"
        ;
    ...
    if(strcmp(argv[2], "requestjoin")==0)
    {
        params->command=action_request_join;
        parse_request_join_cli(argc, argv, params);
    } else if(strcmp(argv[2], "processresponse")==0)
    {
        params->command=action_process_response;
        parse_process_response_cli(argc, argv, params);
...
void parse_issuer_cli(int argc, char** argv, struct cli_params *params)
{
    const char *usage_str =
        "Options:\n"
        "\tcreategroup                   Create a issuer public and secret key.\n"
        "\trespondtorequest              Respond to join request by member.\n"
        ;
    ...
    if(strcmp(argv[2], "creategroup")==0)
    {
        params->command=action_create_group;
        parse_create_group_cli(argc, argv, params);
    } else if(strcmp(argv[2], "respondtorequest")==0)
    {
        params->command=action_respond_to_request;
        parse_respond_to_request_cli(argc, argv, params);
    } else
    {
...
void parse_cli(int argc, char** argv, struct cli_params *params)
{
    const char *usage_str =
        ...
        "\textractgpk                    Extract the GPK from the issuer public key.\n"
        "\tverify                        Verifies that signature is valid.\n"
        ;
